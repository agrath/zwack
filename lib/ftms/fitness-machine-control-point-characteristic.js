...
const Reset = 0x01;
const SetTargetSpeed = 0x02;
const SetTargetInclination = 0x03;
const SetTargetResistanceLevel = 0x04;
...
class FitnessMachineControlPointCharacteristic extends bleno.Characteristic {
  constructor(messages, fmsc, onTargetChanged) {
  ...
  onWriteRequest(data, offset, withoutResponse, callback) {
    ...
    switch (code) {
      case RequestControl:
        ...
        //https://github.com/WouterJD/FortiusANT/pull/409/commits/98b81921b77b88e667ece5a2e672cde9e8ab997a
        if (this.hasControl) {
          debugFTMS('Warning: already has control');
        }
        else {
      ...
      case Reset:
        ...
        if (this.hasControl) {
          ...
          this.hasControl = false;
          //https://github.com/WouterJD/FortiusANT/pull/409/commits/cce23c562f5e80eccacef88d56018a9f3de1e0a4
          this.isStarted = false;
          response = this.result(code, Success);
        ...
        break;
      case SetTargetSpeed:
        debugFTMS('[' + FitnessMachineControlPoint + '][FitnessMachineControlPointCharacteristic] onWriteRequest: Set target speed');
        if (this.hasControl) {
          // Divide (not * 0.01) to avoid float artifacts: 330 * 0.01 =
          // 3.3000000000000003, whereas 330 / 100 = 3.3.
          let targetSpeed = data.readUInt16LE(1) / 100;
          debugFTMS('Target Speed(km/h): ' + targetSpeed);
          response = this.result(code, Success);
          this.fmsc.notifyTargetSpeedChanged(targetSpeed);
          this.onTargetChanged({ type: 'speed', value: targetSpeed });
        }
        else {
          debugFTMS('Error: no control');
          response = this.result(code, ControlNotPermitted);
      ...
      case SetTargetInclination:
        debugFTMS('[' + FitnessMachineControlPoint + '][FitnessMachineControlPointCharacteristic] onWriteRequest: Set target inclination');
        if (this.hasControl) {
          let targetInclination = data.readInt16LE(1) / 10;
          debugFTMS('Target Inclination(%): ' + targetInclination);
          response = this.result(code, Success);
          this.fmsc.notifyTargetInclinationChanged(targetInclination);
          this.onTargetChanged({ type: 'inclination', value: targetInclination });
        }
        else {
          debugFTMS('Error: no control');
          response = this.result(code, ControlNotPermitted);
      ...
      case SetTargetResistanceLevel:
        debugFTMS('[' + FitnessMachineControlPoint + '][FitnessMachineControlPointCharacteristic] onWriteRequest: Set target resistance level');
        if (this.hasControl) {
          let targetResistance = data.readUInt8(1) / 10;
          debugFTMS('Target Resistance: ' + targetResistance);
          response = this.result(code, Success);
          this.fmsc.notifyTargetResistanceChanged(targetResistance);
          this.onTargetChanged({ type: 'resistance', value: targetResistance });
        }
        else {
          debugFTMS('Error: no control');
          response = this.result(code, ControlNotPermitted);
      ...
      case SetTargetPower:
        if (this.hasControl) {
          ...
          this.onTargetChanged({ type: 'power', value: targetPower });
        }
        else {
          debugFTMS('Error: no control');
          response = this.result(code, ControlNotPermitted);
      ...
      case SetIndoorBikeSimulation:
        ...
        if (this.hasControl) {
          let windSpeed = data.readInt16LE(1) / 1000;
          let grade = data.readInt16LE(3) / 100;
          let crr = data.readUInt8(5) / 10000;
          let cw = data.readUInt8(6) / 100;
