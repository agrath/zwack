...
const TargetSpeedChanged = 0x05;
const TargetInclinationChanged = 0x06;
...
class FitnessMachineStatusCharacteristic extends bleno.Characteristic {
  ...
  notifyTargetSpeedChanged(targetSpeed) {
    debugFTMS('[' + FitnessMachineStatus + '][FitnessMachineStatusCharacteristic] notifyTargetSpeedChanged');
    let buffer = new Buffer.alloc(3);
    buffer.writeUInt8(TargetSpeedChanged);
    buffer.writeUInt16LE(Math.round(targetSpeed / 0.01), 1);
    this.notify(buffer);
  }

  notifyTargetInclinationChanged(targetInclination) {
    debugFTMS('[' + FitnessMachineStatus + '][FitnessMachineStatusCharacteristic] notifyTargetInclinationChanged');
    let buffer = new Buffer.alloc(3);
    buffer.writeUInt8(TargetInclinationChanged);
    buffer.writeInt16LE(Math.round(targetInclination / 0.1), 1);
    this.notify(buffer);
  }

  notifyTargetResistanceChanged(targetResistance) {
    debugFTMS('[' + FitnessMachineStatus + '][FitnessMachineStatusCharacteristic] notifyTargetResistanceChanged');
    let buffer = new Buffer.alloc(2);
    buffer.writeUInt8(TargetResistanceLevelChanged);
    buffer.writeUInt8(Math.round(targetResistance / 0.1), 1);
    this.notify(buffer);
  }
