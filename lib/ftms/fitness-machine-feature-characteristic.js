...
const SpeedTargetSettingSupported = bit(0);
const InclinationTargetSettingSupported = bit(1);
...
class FitnessMachineFeatureCharacteristic extends bleno.Characteristic {
  ...
  onReadRequest(offset, callback) {
    ...
    // Power target (ERG) is a trainer/rower concept, not a treadmill one — seed
    // to 0 and add it only where it applies, rather than unconditionally.
    let targetSettings = 0;
    if (containsFTMSBike) {
      targetSettings |= PowerTargetSettingSupported | ResistanceTargetSettingSupported | IndoorBikeSimulationParametersSupported;
    }
    if (containsFTMSTreadmill) {
      targetSettings |= SpeedTargetSettingSupported | InclinationTargetSettingSupported;
    }
    if (containsFTMSRow) {
      targetSettings |= PowerTargetSettingSupported;
    }
    flags.writeUInt32LE(targetSettings, 4);
