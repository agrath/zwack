...
class IndoorBikeDataCharacteristic extends Bleno.Characteristic {
  constructor() {
    ...
    //with reference to https://github.com/TrackMyIndoorWorkout/TrackMyIndoorWorkout/blob/develop/lib/devices/device_descriptors/life_fitness_bike_descriptor.dart

    // FTMS spec order: bit1 = Average Speed, bit2 = Instantaneous Cadence.
    // (The oesmith XML linked above has these two transposed; real trainers and
    // the FTMS spec put cadence at bit2, so a consumer reading per-spec would
    // otherwise decode zwack's cadence as average speed.)
  ...
  notify(event) {
    ...
    //flag bit 0 more_data is negated, if more_data=false then instantaneous_speed is present
    offset += 2;
    ...
    //the xml spec says these are mandatory but the protocol uses byte squishing so the bytes shall only be written if flags on
    ...
    //byte-squishing means we don't advance
    //offset += 4;
    ...
    if ("heart_rate" in event) {
      ...
      debugFTMS("[IndoorBikeDataCharacteristic] heart rate (bpm): " + heart_rate);
