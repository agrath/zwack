...
class TreadmillDataCharacteristic extends Bleno.Characteristic {
 ...
 convertMetersPerSecondToKilometersPerMinute(speedInMetersPerSecond) {
  ...
  notify(event) {
    ...
    // 00000010 00000000 - 0x0200 - Metabolic Equivalent present
    // 00000100 00000000 - 0x0400 - Elapsed Time present
    // 00001000 00000000 - 0x0800 - Remaining Time present
    // 00010000 00000000 - 0x1000 - Force on Belt and Power Output present
    ...
    if ("inclination" in event) {
      var inclination = Math.round(event.inclination * 10); // FTMS spec: 0.1% resolution
      buffer.writeInt16LE(inclination, offset);
      offset += 2;

      var ramp_angle = Math.round(event.ramp_angle * 10); // FTMS spec: 0.1 degree resolution
