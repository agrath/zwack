...
class TreadmillDataCharacteristic extends Bleno.Characteristic {
 ...
 updateDistance(speed, lastTimestamp, currentTimestamp) {
    ...
    return totalDistance; // Optionally return the updated total distance
  }
 ...
 convertMetersPerSecondToKilometersPerMinute(speedInMetersPerSecond) {
  ...
  writeUInt24(buffer, offset, value) {
    ...
    buffer.setUint8(offset, value & 0xFF);             // Least significant byte
    buffer.setUint8(offset + 1, (value >> 8) & 0xFF);  // Middle byte
    buffer.setUint8(offset + 2, (value >> 16) & 0xFF); // Most significant byte
  }
  ...
  notify(event) {
    ...
    this.averageSpeed = this.calcRunningAvg(this.averageSpeed, event.speed, counter);
    ...
    event.instantaneous_pace = convertMetersPerSecondToKilometersPerMinute(event.speed) * 10; //m/s we want km/m *10
    event.average_pace = convertMetersPerSecondToKilometersPerMinute(this.averageSpeed) * 10
