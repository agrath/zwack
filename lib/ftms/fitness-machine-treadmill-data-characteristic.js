 ...
 convertMetersPerSecondToKilometersPerMinute(speedInMetersPerSecond) {
  ...
  notify(event) {
    ...
    if ("inclination" in event) {
      ...
      buffer.writeInt16LE(offset, inclination, true); // 'true' specifies little-endian
      offset += 2;

      var ramp_angle = event.ramp_angle;
      buffer.writeInt16LE(offset, ramp_angle, true); // 'true' specifies little-endian
      offset += 2;
    }

    if ("instantaneous_pace" in event) {
      var instantaneous_pace = event.instantaneous_pace;
      buffer.writeUInt8LE(instantaneous_pace, offset);
      offset += 1;
    }

    if ("instantaneous_pace" in event) {
      var average_pace = event.average_pace;
      buffer.writeUInt8LE(average_pace, offset);
      offset += 1;
    }

    if ("heart_rate" in event && this.treadmillDataFlags.isSet("heart_rate")) {
      //bpm with resolution of 1
      buffer.writeUInt8LE(event.heart_rate, offset);
      offset += 1;
    }
