...
//we use this approximation to work out a power; 28 spm = 250w. Varying the spm just linearly changes the power.
...
class RowerDataCharacteristic extends Bleno.Characteristic {
  ...
  updateDistance() {
    this.totalDistance = Math.round(this.strokeCount * DISTANCE_PER_STROKE);
  ...
  notify(event) {
    ...
    this.power = this.updatePower(event.stroke_rate);
