...
// On Linux, ensure the HCI Bluetooth device is up before bleno initialises.
// Without this, a freshly-booted Raspberry Pi (with bluetoothd disabled) will
// silently fail to advertise because hci0 starts in a "down" state.
function ensureHciUp() {
  if (os.platform() !== "linux") return;

  const deviceId = process.env.BLENO_HCI_DEVICE_ID || "0";
  const device = `hci${deviceId}`;

  try {
    const info = execSync(`hciconfig ${device}`, { encoding: "utf8", timeout: 5000 });
    if (/UP RUNNING/i.test(info)) {
      console.log(`[BLE] ${device} is already up`);
      return;
    }
    console.log(`[BLE] ${device} is down, attempting to bring it up...`);
  } catch {
    // hciconfig not available or device doesn't exist — nothing we can do
    console.warn(`[BLE] Could not query ${device} — hciconfig may not be installed`);
    return;
  }

  try {
    execSync(`hciconfig ${device} up`, { encoding: "utf8", timeout: 5000 });
    console.log(`[BLE] ${device} brought up successfully`);
  } catch (e) {
    console.warn(`[BLE] Failed to bring up ${device}: ${e.message}`);
    console.warn(`[BLE] Run manually: sudo hciconfig ${device} up`);
  }
}
...
var containsRSC = config.has("rsc");
var containsCSP = config.has("csp");
var containsHR = config.has("heartrate");
...
if (containsFTMSBike || containsFTMSTreadmill || containsFTMSRow) {
  servicesOfferedArray.push("1826");
}
if (containsCSP) {
  servicesOfferedArray.push("1818");
}
if (containsRSC) {
  servicesOfferedArray.push("1814");
}
if (containsHR) {
  servicesOfferedArray.push("180D"); //heart rate
  servicesOfferedArray.push("180F"); //battery
}
...
class ZwackBLE extends EventEmitter {
  constructor(options) {
    ...
    this.ftms = new FTMSService([], (target) => {
      this.emit('ftms-target', target);
    });
    ...
    bleno.on("stateChange", (state) => {
      ...
      if (state === "poweredOn") {
        ...
        debugBLE("Stopping...");
        if (state === "poweredOff") {
          console.warn("[BLE] Bluetooth adapter is powered off — waiting for it to come up (polling every 1s)");
          console.warn("[BLE] If this persists, run: sudo hciconfig hci0 up");
        } else if (state === "unauthorized") {
          console.warn("[BLE] Bluetooth access unauthorized — try: sudo setcap cap_net_raw+eip $(which node)");
        }
