...
if ( args.variable === undefined ) {
  ...
  if ( containsFTMSBike || containsFTMSTreadmill ) { servicesOfferedArray[servicesOfferedArray.length] = '1826' }
	...
	constructor(options) {
		...
		bleno.on('advertisingStart', (error) => {
			...
			if (!error) {
				...
				if ( containsFTMSBike || containsFTMSTreadmill ) { services.push(self.ftms); }
				if ( containsCSP ) { services.push(self.csp); }
				if ( containsRSC ) { services.push(self.rsc); }
...
// 			}
	...
	//todo: need to update for treadmill ftms
	notifyFTMS(event) {
		debugFTMS(`[${this.name} notifyFTMS] ${JSON.stringify(event)}`);

		this.ftms.notify(event);

		if (!('watts' in event) &&!('speed' in event) && !('heart_rate' in event)) {
