...
// Messages are buffered because this runs before the TUI is ready.
...
function ensureHciUp() {
  ...
  try {
    ...
    if (/UP RUNNING/i.test(info)) {
      startupMessages.push(`${device} is already up`);
      return;
    }
    startupMessages.push(`${device} is down, attempting to bring it up...`);
  } catch {
    startupMessages.push(`Could not query ${device} — hciconfig may not be installed`);
    return;
  }

  try {
    execSync(`hciconfig ${device} up`, { encoding: "utf8", timeout: 5000 });
    startupMessages.push(`${device} brought up successfully`);
  } catch (e) {
    startupMessages.push(`Failed to bring up ${device}: ${e.message}`);
    startupMessages.push(`Run manually: sudo hciconfig ${device} up`);
  }
}
...
ZwackBLE.startupMessages = startupMessages;
